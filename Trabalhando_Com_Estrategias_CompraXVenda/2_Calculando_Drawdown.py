...
print('Aproveitando código para calcular Drawdown no final do código')
...
print('- - - - - - - - -- - - - - - - - - - - - - - - - - - - - - - - - - -')
print('Objetivo é calcular o Drawdown')
data['Retorno Acumulado'] = data['Retorno Estratégia'].cumsum().apply(np.exp)
data['SomaMax'] = data['Retorno Acumulado'].cummax()
data[['Retorno Acumulado','SomaMax']].dropna().plot(figsize=(20,12))
#plt.show()#Para mostrar o gráfico, descomentar 'plt.show'

print('Calcular o Drawdown')
drawdown = data['SomaMax'] - data['Retorno Acumulado']
rebaixamento = drawdown.max()
print('Rebaixamento: '+ str(rebaixamento)+'%')

print('Por quanto tempo fiquei de Rebaixamento?')
